...
fn from_numpy[dtype: DType, rank: Int](array: PythonObject) raises -> LegacyUnsafePointer[Scalar[dtype]]:
    ...
    dst = LegacyUnsafePointer[Scalar[dtype]].alloc(Int(arr.size))
    memcpy(dst, src, Int(arr.size))
    return dst


struct NDArray[dtype: DType, rank: Int]:
    var data: LegacyUnsafePointer[Scalar[dtype]]

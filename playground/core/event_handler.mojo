...
struct TextInputState:
    ...
    fn start_text_input(mut self, app_state: AppState) raises:
        self.text = ""
        self.cursor = 0
    ...
    fn stop_text_input(mut self, app_state: AppState) raises:
...
struct EventHandler:
    ...
    var tex_callback: fn (mut state: AppState, s: String) raises
    var text_input_state: TextInputState

    fn __init__(out self, tex_callback: fn (mut state: AppState, s: String) raises):
        self.dragging = False
        self.drag_offset_x = 0.0
        self.drag_offset_y = 0.0
        self.text_input_state = TextInputState(cursor=0, text="", text_input_active=False)
        self.tex_callback = tex_callback
    
    fn handle_key_down(mut self, mut app_state: AppState, event: KeyboardEvent) raises -> Bool:
        if self.text_input_state.text_input_active:
            if Int(event.scancode) == Int(Scancode.SCANCODE_RETURN) or Int(event.scancode) == Int(Scancode.SCANCODE_ESCAPE):
                ...
                self.tex_callback(app_state, self.text_input_state.text)
    ...
    fn poll_events(mut self, mut app_state: AppState) raises -> Bool:
